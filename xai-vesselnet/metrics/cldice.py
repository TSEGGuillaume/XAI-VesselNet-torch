...
class clDiceHelper:
    """
    Compute the centerlineDice (clDice) score between two tensors `y_pred` and `y`.
        ...
        for b in range(y_pred.shape[0]):
            ...
            for c in range(first_ch, n_pred_ch) if n_pred_ch > 1 else [1]:
                ...
                if y[b, 0].ndimension() == 2:
                    ...
                    from skimage.morphology import skeletonize_3d
                    sk_x_pred = torch.from_numpy(skeletonize_3d(x_pred)).bool()
                    sk_x = torch.from_numpy(skeletonize_3d(x)).bool()
                else:
