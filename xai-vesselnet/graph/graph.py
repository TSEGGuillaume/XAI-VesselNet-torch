class CNode:
    ...
    """
    def __init__(self, p_id, p_pos: tuple, p_degree: int, p_avgDistToSurface:float = None):
        self._id = p_id
        self.pos = p_pos
    ...
    def __eq__(self, p_node):
        return type(p_node) == type(self) and self._id == p_node._id and self.pos == p_node.pos and self.degree == p_node.degree and self.avgDistToSurface == p_node.avgDistToSurface

    def __repr__(self):
        return "CNode (id: {}, position: {}, degree: {}, avgDistanceToSurface: {})".format(self._id, self.pos, self.degree, self.avgDistToSurface)
...
class CEdge:
    ...
    def getDistance(self, norm_type: str = "L2"):
        ...
        Parameters
            norm_type (str) : The distance method to use.
...
class CCenterline(CEdge):
    ...
    Parameters
        ...
        p_node1 (CNode)             : The first end point/node
        p_node2 (CNode)             : The second end point/node
        p_skeleton_points (list)    : The list of points (dict) that form the skeleton of the centerline. None (default)
    ...
    def getDistance(self, norm_type: str = "default"):
        ...
        if norm_type == "default":
            ...
            pos_chain = [self.node1.pos] + [skpt["pos"] for skpt in self.skeleton_points] + [self.node2.pos]
            for idx_x1, x2 in enumerate(pos_chain[1:]):
                dist += distance(pos_chain[idx_x1], x2, "L2")
        else:
    ...
    def getMidPoint(self) -> CNode:
        ...
        return CNode("skvx_{}".format(self._id), middle_point["pos"], 2, p_avgDistToSurface = middle_point["avgDistToSurface"])
