...
class CProcessThicknessInput:
    ...
    def save_results(self, patch_eedt, patch_info, patch_pos, input_file):
...
def create_patch_info_json(file, in_dir_attribution, I, meta, input_shape, graph, image_saver, out_dir_json):
    fpath = os.path.join(in_dir_attribution, file)
    patch_pos = read_path_position_from_file(fpath)

    struct_fname = Filename(file)
    fname_prefix = struct_fname.get_prefix()

    # Update metadata
    meta_cpy = deepcopy(meta) # I don't think this is necessary as every Process as it's own copy of the obj, but nevermind
    # Update the filename
    meta_cpy["filename_or_obj"] = f"{fname_prefix}.nii.gz"
    # Update the affine
    image_space_new_origin = np.array(patch_pos[0])
    ...
    # Get POI position
    landmark = GetLandmark(graph=graph, landmark_type=struct_fname.landmark_type, landmark_id=struct_fname.landmark_id)

    # Extract patch
    patch_ytrue = np.squeeze(extract_patch(I, patch_pos, input_shape).numpy(), axis=0)

    res = []

    lbl_patch_ytrue, n_vessels_cc = label(patch_ytrue, connectivity=None, return_num=True)

    # Save labeled patch
    image_saver(
        monai.data.MetaTensor(lbl_patch_ytrue, meta=meta_cpy)
    )

    for k in range(1, n_vessels_cc+1):
        vessel_cc = lbl_patch_ytrue == k
        vessel_area = np.count_nonzero(vessel_cc)
    ...
    # Get vessel label that contains the landmark    
    relative_position = tuple(ldmrk_pos - p_pos for ldmrk_pos, p_pos in zip(landmark.pos, patch_pos[0]))
    landmark_vessel_id = lbl_patch_ytrue[relative_position]

    with open(os.path.join(out_dir_json, f"{fname_prefix}_patch_info.json"), "w") as json_f:
        json.dump(
            convert_typing_to_native(
                {
                    "landmark_vessel_id": landmark_vessel_id,
                    "vessels_area": np.count_nonzero(lbl_patch_ytrue),
                    "count_vessels_cc": n_vessels_cc,
                    "vessels_cc": res,
                }
            ),
            json_f,
...
def task_create_patch_info_json(obj_queue, obj_finished_queue, in_dir_attribution, I, meta, input_shape, graph, image_saver, out_dir_json):
    while True:
        try:
            ...
            create_patch_info_json(task_file, in_dir_attribution, I, meta, input_shape, graph, image_saver, out_dir_json)
            obj_finished_queue.put(task_file)
    
    return True


def distribute_patch_info_json_creation(files, in_dir_attribution, I, meta, input_shape, graph, image_saver, out_dir_json, process_count):
    nfiles = len(files)
    ...
    for k in range(process_count):
        ...
        processes.append(
            Process(target=task_create_patch_info_json, args=(files_to_process, files_finished, in_dir_attribution, I, meta, input_shape, graph, image_saver, out_dir_json))
        )
        processes[-1].start()
...
def main(in_dir_attribution, in_dir_ytrue, in_dir_graphs, hyperparameters_path, output_dir, process_count=None):
    ...
    if not is_patch:
        raise ValueError(f"Patch information can only be computed for patched data. Patched data ? {is_patch}")
    ...
    out_dir_skeleton, out_dir_json = create_output_dirs(
        [
            os.path.join(output_dir, "patch_info", "cc"),
            os.path.join(output_dir, "patch_info", "json"),
        ],
        attribution_id
    )
    ...
    for sample in unique_samples:
        ...
        distribute_patch_info_json_creation(sample_files, in_dir_attribution, I, meta, input_shape, graph, image_saver, out_dir_json, process_count)
